...
public class MerkleTree {
    ...
    private List<TreeNode> createParentList(List<TreeNode> leafList){
        ...
        //空检验
        if(leafList == null || leafList.size() == 0) return parents;
    ...
    private TreeNode createParentNode(TreeNode left, TreeNode right){
        ...
        parent.setData(hash);
        parent.setHash(hash);
        if(right != null) parent.setName("(" + left.getName() + "和" + right.getName() + "的父节点)");
        else parent.setName("(继承节点（" + left.getName() + ")成为父节点）");
        return parent;
    }
    ...
    private List<TreeNode> createLeafList(List<String> contents){
        ...
        if(contents == null || contents.size() == 0) return leafList;
    ...
    private void traverseTreeNodes(TreeNode node){
        ...
        if(node.getLeft() != null) traverseTreeNodes(node.getLeft());
        if(node.getRight() != null) traverseTreeNodes(node.getRight());
    }
